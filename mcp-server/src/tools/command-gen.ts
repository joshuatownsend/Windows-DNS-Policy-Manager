...
/**
 * Offline PowerShell command generation — works without the bridge.
...
function generateCommands(
  ...
  switch (scenarioId) {
    ...
    case "geolb": {
      ...
      const glDcs: any[] = data.geolbDatacenters || [];
      ...
      glRegs.forEach((r: any) => {
        if (!r.name || !r.subnet) return;
        const rW = glW[r.name] || {};
        const se = glDcs.filter((d: any) => d.name).map((d: any) => `${d.name}ZoneScope,${rW[d.name] || 1}`);
      ...
      if (data.geolbWorldwide !== false) {
        cmds.push("# Worldwide catch-all");
        const glCa = glDcs.filter((d: any) => d.name).map((d: any) => `${d.name}ZoneScope,1`);
...
export function registerCommandGenTools(server: McpServer) {
  server.tool(
    "dns_generate_policy_commands",
    `Generate PowerShell commands for common DNS policy scenarios. Works OFFLINE — no bridge needed.
Supported scenarios: geolocation, splitbrain, blocklist, timeofday, loadbalancing, geolb, primarysecondary, queryfilter.

Example data for 'blocklist': { "blocklistDomains": "malware.com,phishing.net", "blocklistAction": "IGNORE" }
Example data for 'geolocation': { "zone": "contoso.com", "recordName": "www", "regions": [{ "name": "US", "subnet": "10.0.0.0/8", "ip": "1.2.3.4" }] }
